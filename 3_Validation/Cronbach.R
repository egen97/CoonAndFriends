...
chatgpt1 <- readRDS("./3_Validation/3_Intercoder_Reliability/ChatGPT_1/completions_all.rds") %>%
...
chatgpt2 <- readRDS("./3_Validation/3_Intercoder_Reliability/ChatGPT_2/completions_df.rds") %>%
  dplyr::select(rowid, ukraine, west, putin, support, sentiment, trust, competence, state_of_war, responsibility, response) %>%
  rename_all(str_to_lower) %>%
  mutate_all(as.numeric) %>%
...
chatgpt3 <- readRDS("./3_Validation/3_Intercoder_Reliability/ChatGPT_3/completions_df_final.rds") %>%
  dplyr::select(rowid, ukraine, west, putin, support, sentiment, trust, competence, state_of_war, responsibility, response) %>%
  rename_all(str_to_lower) %>%
  mutate_all(as.numeric) %>%
...
chatgpt4 <- readRDS("./3_Validation/3_Intercoder_Reliability/ChatGPT_4/completions_df_all.rds") %>%
  dplyr::select(rowid, ukraine, west, putin, support, sentiment, trust, competence, state_of_war, responsibility, response) %>%
  rename_all(str_to_lower) %>%
  mutate_all(as.numeric) %>%
...
coded_vars <- c("support", "sentiment", "trust", "competence")
#  "ukraine", "west", "putin",
